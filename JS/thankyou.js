...
document.getElementById('brief-display').innerHTML = `
  ...
  <div class="brief-section">
    ...
    <div class="fields-grid">
      ...
      ${data.logoImage ? `<div class="field full"><label>Logo File</label><img src="${data.logoImage}" class="logo-preview-display"></div>` : ''}
...
function downloadPDF() {
  ...
  doc.text('Mohamed Ali Louati — Graphic & Brand Identity Designer', 105, 20);
  ...
  // Logo image في الـ PDF
  if (data.logoImage) {
    if (y > 220) { doc.addPage(); y = 20; }
    doc.setTextColor(...gray);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('LOGO FILE', 16, y);
    y += 5;
    try {
      doc.addImage(data.logoImage, 16, y, 60, 40);
    } catch(e) {
      doc.setTextColor(...black);
      doc.text('Logo image attached', 16, y + 10);
    }
    y += 50;
  }
